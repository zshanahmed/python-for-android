...
class OscpyRecipe(Recipe):
    ...
    def prebuild_arch(self, arch):
        super(OscpyRecipe, self).prebuild_arch(arch)
        ...
        info("Cloning oscpy sources from {}".format(self.oscpy_git))
        shprint(sh.git,
                'clone', '-b',
                self.oscpy_branch,
                '--depth=1',
    ...
    def build_arch(self, arch):
        super(OscpyRecipe, self).build_arch(arch)
    ...
    def postbuild_arch(self, arch):
        super(OscpyRecipe, self).postbuild_arch(arch)
